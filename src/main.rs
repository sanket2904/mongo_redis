...
fn find_primary(mongouri: &str) -> String {
    ...
    let main_ip = format!("{}:{}", main_ip[0..main_ip.len() - 1].to_string(), ip.port());
    ...
    let primary = document.get("primary").unwrap().as_str().unwrap().to_owned();
    primary
...
pub fn start_main(listen_addr: String, port: u16) {
    ...
    for stream in listner.incoming() {
        ...
        let config = ClientConfig::builder()
            ...
            .with_no_client_auth();
        let arc = std::sync::Arc::new(config);
        let a = addr.to_owned().split(":").collect::<Vec<&str>>()[0].to_string();
        ...
        pool.execute(move || {
            ...
            handle_connection(stream, client, server, &storage.data);
        });
    }
...
fn handle_connection(
    ...
    let mongo_client: rustls::Stream<'_, rustls::ClientConnection, TcpStream> = rustls::Stream::new(&mut client, &mut server);
    // throw error if tls handshake fails
    let mongo_client: Arc<Mutex<rustls::Stream<'_, rustls::ClientConnection, TcpStream>>> = Arc::new(Mutex::new(mongo_client));
    loop {
