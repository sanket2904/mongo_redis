...
impl ListDatabases {
    ...
    pub fn  handle(&self,_request: &crate::handler::Request<'_>,msg: &Vec<bson::Document>) -> Result<bson::Document, crate::handler::CommandExecutionError> {
        ...
        let res = send_databases(name_only,mongo_client);

        return res;
        
    }
}
fn send_databases(name_only: bool , mongo_client: &mongodb::sync::Client) -> Result<bson::Document, crate::handler::CommandExecutionError> {
    let mut databases: Vec<bson::Bson> = vec![];
    // let mongo_client = MongoDb::new().await;
    let list =  mongo_client.list_databases(None, None).unwrap();
